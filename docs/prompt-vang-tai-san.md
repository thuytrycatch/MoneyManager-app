...
- Ví nằm ở bảng `public.accounts` (xem `supabase-schema.sql`). Mỗi ví có `class` = `asset|liability`.
- Số dư & net worth tính **client-side** trong `js/app.js`:
  - `accountBalance(id)` (~L786): `opening_balance + Σ giao dịch`.
  - `accountClass(acc)` (~L782), `netWorth()` (~L807): `assets − liabilities`.
  - `netWorthHtml()` (~L1562): thẻ hiển thị Net worth.
  - Hằng `ACCOUNT_TYPES`, `LIABILITY_TYPES`, `ACCOUNT_TYPE_META` (~L767–778).
  - Editor ví trong Cài đặt: `walletEditRowHtml(acc)` (~L1797) + handler Save (~L2332) + toggle credit fields (~L2300).
  - Tiền hiển thị qua `fmtShort` / `fmtVND` (`js/charts.js`); ẩn số dư qua `mask()`.
- Tầng dữ liệu: `js/store.js` — `mapAccount` (~L500/320), `addAccount` (~L496), `updateAccount` (~L526), realtime subscribe (~L658).
- App **đã** gọi Claude API trực tiếp từ browser: `js/parser.js:202` (header `anthropic-dangerous-direct-browser-access: true`). Dùng lại pattern này cho Phase 2 (web_search).
...
3. Thêm `goldPrices` vào payload `loadAll()`:
   - `select('*').from('gold_prices')` → trả về object `{ [kind]: { buyPerChi, sellPerChi, source, fetchedAt } }`.
   - Đưa vào `DATA.goldPrices`.
4. Subscribe realtime bảng `gold_prices` (giống các bảng khác ~L658) để khi giá đổi, UI tự refresh.
...
```
Sửa **`accountBalance(id)`**: ngay đầu hàm, nếu `acc.type === 'gold'` → `return goldValue(acc);` (bỏ qua opening_balance + giao dịch). Nhờ vậy `netWorth()`, `totalBalance()`, mọi nơi gọi `accountBalance` tự động đúng — vàng vào **Tổng tài sản** mà không cần sửa thêm. (Lưu ý: net worth dùng **giá trị hiện tại**, không phải giá gốc — đúng bản chất tài sản.)
...
- Khi `type === 'gold'`: thay khối credit-fields bằng **khối gold-fields** (CSS class `.wallet-gold-fields`, ẩn/hiện theo type giống credit):
  ...
  - Dòng giá trị live: `= fmtShort(goldValue(acc))` + `(giá X/chỉ · cập nhật lúc <fetched_at>)`.
...
- Trong handler Save (~L2332): khi `type==='gold'`, build `extra` gồm `class:'asset'`, `goldWeightChi`, `goldKind`, `goldFactor` (từ % → số), `goldCustomBuy` (chỉ khi custom, ngược lại `null`), `goldBuyPerChi`, `goldBuyDate` (rỗng → `null`); và clear `creditLimit/statementDay/dueDay = null`. Với type khác gold: set các field gold về `null`/`1` để không lẫn.
- Cập nhật listener toggle (~L2300): khi đổi `.w-type`, hiện `.wallet-gold-fields` nếu `=== 'gold'`, hiện `.wallet-credit-fields` nếu là liability, ẩn còn lại.
...
- Tạo `supabase/functions/gold-price/index.ts` (Deno). Server-side fetch một nguồn giá VN
  (vd API JSON của SJC, hoặc trang webgia… — fetch ở server nên không vướng CORS), parse ra
  `buy_per_chi`/`sell_per_chi` cho từng `kind`, rồi **upsert** vào `public.gold_prices` bằng
  service-role key (đọc từ `Deno.env`). Trả về JSON các giá vừa cập nhật.
- Lên lịch bằng Supabase Scheduled Functions (cron, vd mỗi 1–4h) để giá luôn mới; client chỉ
  cần đọc bảng `gold_prices` (realtime tự đẩy về). Nút refresh ở UI gọi function này on-demand.
- Hàm client `refreshGoldPrices()` = `fetch('<project>.functions.supabase.co/gold-price', { headers: { Authorization: 'Bearer '+anonKey }})` rồi `await refreshData()`.
- Ưu điểm: chính xác (nguồn VN), không lộ logic/khoá ở client, cả nhà dùng chung cache.
...
- ✅ Tự quy đổi ra VND theo giá mua vào hiện tại; hiện thời điểm cập nhật + cảnh báo giá cũ.
